(ns midas-loop.xtdb.scratch
  (:require [xtdb.api :as xt]
            ...
            [midas-loop.xtdb.queries.diff :as cxqd]
            [midas-loop.server.tokens :as tok]
            [editscript.core :as e]
            [midas-loop.conllu-parser :as cp])
...
(comment
  ...
  (def xs (midas-loop.conllu-parser/parse-conllu-string data))
  xs

  (require '[midas-loop.server.xtdb :refer [xtdb-node]])

  (cxc/create-document node xs)

  (def doc-id (ffirst (xt/q (xt/db node) {:find '[?d] :where [['?d :document/id]]})))

  (spit "/tmp/bar" (midas-loop.xtdb.serialization/serialize-document node doc-id))
  (spit "/tmp/bar" (midas-loop.xtdb.serialization/serialize-document xtdb-node #uuid "b066f4d4-cce5-4f04-93f9-702ae5912bc5"))
...
(comment
  (let [a two-sentence-minimal
    ...
    (def xs (midas-loop.conllu-parser/parse-conllu-string a))
    (cxc/create-document node xs)
  ...
  (require '[midas-loop.xtdb-diff-test :as cxd])
...
(comment
  ;; tokens
  (require '[midas-loop.server.tokens :as tok :refer [xtdb-token-node create-token]])
